...
// ─── Config dos formulários por módulo ────────────────────────────────────────
...
export default function AbaAvaliacaoModulo({ pacienteId, modulo }: Props) {
  ...
  const camposModulo: CampoConfig[] = CAMPOS_AVALIACAO[modulo] ?? []
      ...
      {!avaliacao && !formularioAberto && (
        <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center">
          ...
          <p className="text-xs text-gray-400 mb-4">
            Registre a avaliação inicial para {config?.label}
          </p>
          <button
            onClick={() => setFormularioAberto(true)}
        ...
        {formularioAberto && (
          ...
          >
            ...
            <div className="space-y-4">
              ...
              {camposModulo.map(({ campo, label, opcoes }) => (
                <div key={campo}>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#1B4332]/20 focus:border-[#1B4332] bg-white cursor-pointer"
                  >
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">
                  Observações gerais <span className="font-normal normal-case text-gray-400">(opcional)</span>
                </label>
                <textarea
